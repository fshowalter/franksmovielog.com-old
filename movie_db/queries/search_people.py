...
class PersonSearchResult(object):
...
def search_people(query: str) -> List[PersonSearchResult]:
...
def _fetch_results(
  ...
  query: str,
) -> Tuple[List[PersonSearchResult], Set[str]]:  # noqa: WPS221
...
def _parse_rows(rows: List[Any]) -> Tuple[List[PersonSearchResult], Set[str]]:
    search_results: List[PersonSearchResult] = []
    title_ids: Set[str] = set()

    for row in rows:
        search_result = PersonSearchResult(row)
